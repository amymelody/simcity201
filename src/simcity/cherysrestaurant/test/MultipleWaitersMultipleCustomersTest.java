...
public class MultipleWaitersMultipleCustomersTest extends TestCase
{
	...
	public void setUp() throws Exception
	{
		...
		w1 = new MockWaiter("W1");
		w2 = new MockWaiter("W2");
		c1 = new MockCustomer("C1");
		c2 = new MockCustomer("C2");
	}	
	/**
	...
	//Preconditions	
		...
		assertEquals("W2 should have an empty event log before the Cashier's scheduler is called. Instead, M2's event log reads: "
				+ w2.getLog().toString(), 0, w2.getLog().size());
	...
	//Step 1 for C2
		cashier.msgProduceCheck(w2, "Pizza", 2); //send the message from W2
		...
		assertEquals("W1 should have an empty event log after the Cashier's scheduler is called for the first time. Instead, W1's event log reads: "
				+ w1.getLog().toString(), 0, w1.getLog().size());
		assertEquals("W2 should have an empty event log after the Cashier's scheduler is called for the first time. Instead, W2's event log reads: "
				+ w2.getLog().toString(), 0, w2.getLog().size());
	...
	//Step 1 for C1
		...
		assertEquals("W1 should have an empty event log after the Cashier's scheduler is called for the second time. Instead, W1's event log reads: "
				+ w1.getLog().toString(), 0, w1.getLog().size());
		assertEquals("W2 should have an empty event log after the Cashier's scheduler is called for the second time. Instead, W2's event log reads: "
				+ w2.getLog().toString(), 0, w2.getLog().size());
	...
	//Step 2 for C2
		cashier.msgGiveCheck(w2, 2); //send the message from W2
		...
		assertTrue("Checks should contain a bill with state == askedFor. It doesn't.",
				cashier.checks.get(0).state == CashierAgent.CheckState.askedFor);
		assertTrue("Cashier's scheduler should have returned true (needs to react to customer's msgGiveCheck), but didn't.", 
				cashier.pickAndExecuteAnAction());
		assertTrue("W2 should have logged \"Received msgHereIsCheck\" but didn't. His log reads instead: " 
				+ w2.getLog().getLastLoggedEvent().toString(), w2.getLog().containsString("Received msgHereIsCheck from cashier. Order = Pizza. Total = 8.99"));
		//Checking logs
		...
		assertEquals("W1 should have an empty event log after the Cashier's scheduler is called for the third time. Instead, W1's event log reads: "
				+ w1.getLog().toString(), 0, w1.getLog().size());
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the third time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
	...
	//Step 3 for C2
		cashier.msgPayment(c2, w2.getCheck(), 9.0); //send the message from a customer
		...
		assertTrue("Checks should contain a bill with state != paid. It doesn't.",
				cashier.checks.get(0).state != CashierAgent.CheckState.paid);
		...
		assertTrue("Checks should contain a bill with the right customer in it. It doesn't.", 
				cashier.checks.get(0).customer == c2);
		...
		assertEquals("W1 should have an empty event log after the Cashier's scheduler is called for the fourth time. Instead, W1's event log reads: "
				+ w1.getLog().toString(), 0, w1.getLog().size());
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the fourth time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
	...
	//Step 4 for C2
		...
		assertEquals("W1 should have an empty event log after the Cashier's scheduler is called for the fifth time. Instead, W1's event log reads: "
				+ w1.getLog().toString(), 0, w1.getLog().size());
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the fifth time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
	...
	//Step 2 for C1
		...
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the sixth time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
	...
	//Step 3 for C2
		...
		assertEquals("W1 should have one event logged after the Cashier's scheduler is called for the seventh time. Instead, it has: "
				+ w1.getLog().toString(), 1, w1.getLog().size());
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the seventh time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
	...
	//Step 4 for C1
		...
		assertEquals("W1 should have one event logged after the Cashier's scheduler is called for the eigth time. Instead, it has: "
				+ w1.getLog().toString(), 1, w1.getLog().size());
		assertEquals("W2 should have one event logged after the Cashier's scheduler is called for the eigth time. Instead, it has: "
				+ w2.getLog().toString(), 1, w2.getLog().size());
